...
class test_data_generator(keras.utils.Sequence):
    ...
    def __init__(self, dataset_folder_list, model, stride_ratio, flip=True, rotate=True, std_normalization=True, roi_folder="", roi_tag="_roi"):
        'Initialization'
        ...
        if model.input_shape[1]==model.input_shape[2]:
            ...
            self.frame_number = model.input_shape[2]
            self.channel_axis = 1
            self.h_axis = 2
            self.w_axis = 3
            self.catagory_number = model.output_shape[1]

        self.patch_size = [model.input_shape[self.h_axis],model.input_shape[self.w_axis]]
        self.stride_length = int(np.max((1,np.round(stride_ratio*np.min(self.patch_size)))))

        self.data_info = []
        self.current_data = {'current_path':[],'current_frame_idx':[]}
        self.predict_output = {}
        self.predict_roi_masks = {}
            ...
            for current_image_filename in tqdm(image_filename_list, desc='Files'):
                ...
                if current_image_filename_ext in [".tif"]:
                    ...
                    if image_stack.series[0].ndim==3:
                        ...
                        break
                    self.predict_output[current_image_path] = np.zeros((image_heigh,image_width,self.catagory_number))

                    # Read roi files 
                    try:
                        roi_path = os.path.join(dataset_folder, roi_folder, current_image_filename_base+roi_tag+".mat")
                        self.roi_file_list[current_image_path] = roi_path
                        ...
                        pass
                        #self.roi_file_list[current_image_path] = []
                        #self.known_masks[roi_path] = []


                    # put everything into data_info
                    for frame_idx in range(image_frame_number-self.frame_number+1):
                        row_idx_list = range(0,image_heigh-self.patch_size[0],self.stride_length)
                        col_idx_list = range(0,image_width-self.patch_size[1],self.stride_length)
                        for row_idx in row_idx_list:
                            for col_idx in col_idx_list:
                                ...
                                data_info_temp.append(data_info)

                                if self.flip:
                                    data_info_temp.append(dict(data_info,**{"h_flip":True,"w_flip":False}))
                                    data_info_temp.append(dict(data_info,**{"h_flip":False,"w_flip":True}))
                                    data_info_temp.append(dict(data_info,**{"h_flip":True,"w_flip":True}))

                                if self.rotate:                                    
                                    for data_info_rotate in data_info_temp.copy():
                                        for rot_idx in range(1,4):
                                            data_info_temp.append(dict(data_info_rotate,**{"rotate":rot_idx}))
                                self.data_info = self.data_info+data_info_temp
